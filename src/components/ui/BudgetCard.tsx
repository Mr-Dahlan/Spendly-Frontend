...
export default function BudgetCard({
  ...
  const { deleteBudget, isLoading: isDeleting } =
    useDeleteBudget();
  ...
  const percent = Math.min(
    budget.usage.percentage ?? 0,
    100,
  );

  const budgetAmount = parseFloat(
    (budget as any).amount_limit || "0",
  );
    ...
    <>
      <div className="bg-[var(--card)] rounded-2xl p-4 shadow-[var(--boxShadow)] border text-[var(--text)] border-gray-100 flex flex-col gap-3">
        ...
        <div className="flex items-center justify-between">
          <span
            className={`text-xs font-semibold px-2 py-0.5 rounded-full ${status.badgeClass}`}
          >
            {status.label}
          </span>
        ...
        <div className="flex items-center justify-between text-xs text-[var(--text-secondary)]">
          <span>
            {budget.usage.start_date &&
            budget.usage.end_date
              ? `${new Date(
                  budget.usage.start_date,
          ...
          <span className="px-2 py-0.5 rounded-full bg-gray-100 text-gray-500 font-medium capitalize">
            {periodLabel[
              budget.usage.period
            ] ?? budget.period}
          </span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl p-6 shadow-xl w-80 flex flex-col items-center gap-4">
            ...
            <p className="text-sm text-gray-500 text-center">
              ...
              Spending data linked to this category
              will remain but won't be tracked against
              a budget.
            </p>
