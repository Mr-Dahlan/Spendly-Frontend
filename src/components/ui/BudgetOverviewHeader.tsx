...
export default function BudgetOverviewHeader({
  ...
  const percent = totalBudgeted > 0 ? Math.min((totalSpent / totalBudgeted) * 100, 100) : 0;
    ...
    <div className="flex flex-col md:flex-row gap-4 mb-6">
      ...
      <div className="bg-[var(--card)] rounded-2xl p-5 shadow-sm border border-gray-100 flex flex-col items-center justify-center min-w-[160px]">
        <svg width="100" height="100" viewBox="0 0 100 100">
          ...
          />
          <text x="50" y="54" textAnchor="middle" fontSize="16" fontWeight="bold" fill="#4f46e5">
            {Math.round(percent)}%
          </text>
        </svg>
        <p className="text-xs text-gray-400 mt-1">Utilized</p>
        <p className="text-sm font-semibold text-indigo-600 mt-0.5">{formatIDR(remaining)} left</p>
      </div>
    </div>
  );
}