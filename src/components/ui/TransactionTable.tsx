...
export default function TransactionTable({
  ...
  const PaginationBar = () =>
    totalEntries > 0 ? (
      <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-t border-[var(--border)] bg-[var(--card)] text-[var(--text)]">
        ...
        {transactions.length === 0 ? (
          ...
          <div className="divide-y divide-gray-100">
            {transactions.map((t) => {
              ...
              const isIncome = t.type === "income";
              return (
                <div key={t.transaction_id} className="px-4 py-3.5 flex items-center gap-3">
                  {/* Left: icon + category */}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className={`inline-flex items-center gap-0.5 px-1.5 py-0.5 rounded-full text-[10px] font-semibold flex-shrink-0 ${TYPE_BADGE[t.type as "income" | "expense"] ?? "bg-gray-100 text-gray-600"}`}>
                        {TYPE_ICON[t.type as "income" | "expense"]}
                        {t.type.charAt(0).toUpperCase() + t.type.slice(1)}
                      </span>
                    </div>
                    ...
                    <p className="text-[11px] text-gray-400 mt-0.5">{formatDate(t.transaction_date)}</p>
                  </div>
                  ...
                  <div className="flex flex-col items-end gap-1.5 flex-shrink-0">
                    ...
                    <div className="flex gap-1">
                      <button
                        onClick={() => onEdit(t)}
                        className="w-7 h-7 rounded-lg bg-gray-100 hover:bg-indigo-100 text-gray-500 hover:text-indigo-600 flex items-center justify-center transition-colors"
                      >
                        <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                        </svg>
                      ...
                      <button
                        onClick={() => onDelete(t)}
                        className="w-7 h-7 rounded-lg bg-gray-100 hover:bg-red-100 text-gray-500 hover:text-red-500 flex items-center justify-center transition-colors"
                      >
                        <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                        </svg>
